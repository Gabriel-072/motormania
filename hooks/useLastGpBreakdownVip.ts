// hooks/useLastGpBreakdownVip.ts
'use client';
...
interface UseLastGpBreakdownVipReturn {
  self?: BreakdownEntry;
  loading: boolean;
  error: string | null;
}

export default function useLastGpBreakdownVip(
  lastGpName: string | null,
  userId?: string
): UseLastGpBreakdownVipReturn {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // 2) Traer DESGLOSE (vip_prediction_breakdown) para este usuario y GP
        const { data: breakdownRow, error: breakdownErr } = await supabase
          .from('vip_prediction_breakdown')
          .select(
