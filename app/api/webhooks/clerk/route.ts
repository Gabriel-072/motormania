...
export async function POST(req: Request) {
  ...
  if (!CLERK_WEBHOOK_SECRET || !APP_URL) { // Check both critical vars
    return NextResponse.json({ error: 'Server configuration error: Missing secrets or URL' }, { status: 500 });
  }
  ...
  if (!svix_id || !svix_timestamp || !svix_signature) { console.warn("Clerk Webhook: Missing Svix headers."); return new NextResponse('Missing Svix headers', { status: 400 }); }
  ...
  } catch (err: any) { console.error('Clerk Webhook: Signature verification failed:', err.message); return NextResponse.json({ 'Error': 'Webhook signature verification failed', details: err.message }, { status: 400 }); }
  ...
  if (eventType === "user.created") {
    ...
    if (!email || !clerk_id) { console.error(`Clerk Webhook Error (user.created): Missing Clerk ID or Email. ID: ${clerk_id}, EmailFound: ${!!email}`); return NextResponse.json({ error: "Required user data missing" }, { status: 400 }); }
    ...
    try {
      ...
      // 1. Upsert clerk_users
      console.log(`Clerk Webhook: Upserting into clerk_users for ${clerk_id}...`);
      const { error: userUpsertError } = await supabase.from("clerk_users").upsert({ clerk_id: clerk_id, email: email, username: username || null, full_name: fullName, updated_at: now }, { onConflict: 'clerk_id' });
      if (userUpsertError) throw new Error(`DB Error (clerk_users upsert): ${userUpsertError.message}`);
      console.log(`Clerk Webhook: User ${clerk_id} synced.`);
      ...
      const { error: entriesUpsertError } = await supabase.from("entries").upsert({ user_id: clerk_id, numbers: initialNumbers, paid_numbers_count: 0, name: fullName, email: email, region: "CO" }, { onConflict: 'user_id', ignoreDuplicates: false });
      if (entriesUpsertError) throw new Error(`DB Error (entries upsert): ${entriesUpsertError.message}`);
      console.log(`Clerk Webhook: Initial entries created/updated for ${clerk_id}.`);

      // 4. Upsert Wallet
      console.log(`Clerk Webhook: Upserting into wallet for ${clerk_id}...`);
      const { error: walletError } = await supabase.from("wallet").upsert({ user_id: clerk_id, mmc_coins: 0, fuel_coins: 0 }, { onConflict: 'user_id', ignoreDuplicates: false });
      if (walletError) { console.warn(`Clerk Webhook DB Warning (Wallet upsert ${clerk_id}):`, walletError.message); } else { console.log(`Clerk Webhook: Wallet entry created/updated for ${clerk_id}.`); }

      // 5. Trigger email API (WITHOUT internal secret)
